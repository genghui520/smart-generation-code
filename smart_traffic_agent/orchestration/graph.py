"""Domain-independent graph builder for AgentTemplate roles."""
...
from .roles import AgentTemplate
...
class AgentGraph:
    ...
    def add_agent_node(self, name: str, agent: AgentTemplate, handler: Callable[[Any], Any]) -> None:
        if name in self._nodes:
            raise ValueError(f"Agent graph node already registered: {name}")
        self._nodes[name] = GraphNodeSpec(name, agent.role_name)
    ...
    def add_conditional_edges(self, source: str, router: Callable[[Any], str], destinations: Mapping[str, str]) -> None:
        self._conditional_edges.append({"source": source, "destinations": dict(destinations)})
    ...
    def describe(self) -> dict[str, Any]:
        return {"nodes": [asdict(node) for node in self._nodes.values()],
